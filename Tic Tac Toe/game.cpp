...
char genRandomTurn();
bool gameRoundLoop(const int roundInfo[ORDERED_PAIR], int score[ORDERED_PAIR], char whoseTurn, int& displayHelpCtr);
int pauseScreen();
...
int getGameInput(const char whoseTurn, int& displayHelpCtr);
...
void gameMasterLoop(const int numOfRounds, int& displayHelpCtr)
{
	...
	while (continuePlaying == true)
	{
		...
		continuePlaying = gameRoundLoop(roundInfo, score, whoGoesFirst, displayHelpCtr);
	}
}
...
bool gameRoundLoop(const int roundInfo[ORDERED_PAIR], int score[ORDERED_PAIR], char whoseTurn, int& displayHelpCtr)
{
	...
	while (gameState == GAMEPLAY_STATE)		//the game runs until an end state is reached, a win, tie or quit state
	{
		...
		input = getGameInput(whoseTurn, displayHelpCtr);
		switch (input)
		{
		case ENTER:
		case SPACEBAR:
...
int getGameInput(const char whoseTurn, int& displayHelpCtr)
{
	...
	do
	{
		...
		if (validInput == false && helpMSGDisplayed == false && displayHelpCtr > 0)
		{
			cout << "\n\n\n\n\n(If you need help, pres Esc to pause the game)\n(Otherwise continue playing)\n";
